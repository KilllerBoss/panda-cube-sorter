...
bool expect(const Weights& w, const SecEntry& s, const char* name, size_t n_floats, std::string& err) {
  if (strncmp(s.name, name, 8) != 0) { err = std::string("section mismatch: got '") +
...
bool Weights::load_from_memory(const uint8_t* d, size_t size) {
  ...
  for (const SecEntry& s : secs) {
    ...
    } else if (strncmp(s.name, "LSNN_W", 8) == 0) {
    ...
    } else if (strncmp(s.name, "EMB_PROJ", 8) == 0) {
    ...
    } else if (strncmp(s.name, "SMOE_W", 8) == 0) {
    ...
    } else if (strncmp(s.name, "HINGE_T2", 8) == 0) {
    ...
    } else if (strncmp(s.name, "LORA_B", 8) == 0) {
